...
/// </summary>
public class ExtendedGrabbable : PointableElement, IGrabbable, ITimeConsumer
{
    /// <summary>
    ...
    private class ThrowWhenUnselected : ITimeConsumer, IDisposable
    {
        ...
        private int _maxLookBack = 10;
        private List<Vector3> _lookBackVelocites = new List<Vector3>();
        private List<Vector3> _lookBackTorques = new List<Vector3>();
        ...
        private void Process(bool forceSubmit)
        {
            ...
            if (time > _lastTime || forceSubmit)
            {
                ...
                GetThrowVelocities(out Vector3 v, out Vector3 t);
                _lookBackVelocites.Add(v);
                _lookBackTorques.Add(t);
                if (_lookBackVelocites.Count > _maxLookBack)
                {
        ...
        private void LoadThrowVelocities()
        {
            ...
            {
                Vector3 velocity = _lookBackVelocites.Aggregate(new Vector3(0, 0, 0), (v1, v2) => v1 + v2) / _lookBackVelocites.Count;
                Vector3 torque = _lookBackTorques.Aggregate(new Vector3(0, 0, 0), (t1, t2) => t1 + t2) / _lookBackTorques.Count;
